...
if (!GROK_API_KEY) {
  console.warn('⚠️ GROK_API_KEY is not set. AI suggestions will not work. Please set GROK_API_KEY in your .env file.')
}
...
export async function getAISuggestions(scanType, scanName, scanResults, rawOutput, target) {
  try {
    ...
    if (!GROK_API_KEY) {
      throw new Error('GROK_API_KEY is not configured. Please set GROK_API_KEY in your .env file.')
    }
